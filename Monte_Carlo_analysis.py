...
rd.seed(1)
for i in range(runs):
    x_4 = rd.gammavariate(50.09483, 0.029265)
    x_2 = rd.uniform(-1.89, -0.59)
    total_discharge = np.zeros(length)
    UH1 = fun_UH1(x_4)
    UH2 = fun_UH2(x_4)
    for t in range(length):
        P_m = precip_lesse[t]
        P = rd.normalvariate(P_m, 0.15*P_m)
        if P < 0:
            P = 0
        E = evap_lesse[t]
        [R, S, Q] = update_timestep(t, P, E, R, S, x_1, x_2, x_3, UH1, UH2)
        total_discharge[t] = Q / 86400 * area * 1000
    results.append([x_2, x_4])
    all_series.append(total_discharge)
    print("Run: ", i)
    # Tussendoor opslaan
    if i in [999, 1999, 2999, 3999, 4999, 5999, 6999, 7999, 8999]:
        np.savetxt("Monte_Carlo.txt", results, '%.2e')
        np.savetxt("All_series.txt", all_series, '%.2e')
